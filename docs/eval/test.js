...
const isTest = true;
const descrPre = "\nHere's an oveview of a data set. It is a hierarchy/tree data structure, where each sentence is preceded by its placement within the tree. For instance, 1.1.2 refers to the second child of the first child of the head:\n";
...
async function clickHandler(isEvaluate) {
    ...
    // Validation Set URL
    const testingSetURL = "https://docs.google.com/spreadsheets/d/e/2PACX-1vQTJFoqwcG96q8V-QH3fN-e33zJMf-mrKwSw3d8OzXrHquwbxUkJQs0j_uSp1F4e1b2RQlTmB0RJG-I/pub?gid=493051093&single=true&output=csv";
    ...
    //         testingSetSample[key] = testingSet[key].slice(0, 25);
    //     }
    // }
    ...
    for (var [key, value] of Object.entries(testingSetSample)) {
        try {
            ...
            const view = await new vega.View(runtime)
                ...
                .hover()
                .runAsync();
            handleDataUpdate(view, vLSpec, isTest);
            ...
            let descrPostRaw = await fetch("/api/get-backend-file?file_path=" + descrPostFilePath);
            let descrPostJSON = await descrPostRaw.json();
...
async function classifyQuestions(value) {
    ...
    for (const classificationObj of value) {
        ...
        const classificationQueryContents = classificationQuery["contents"];
        const classificationResponse = await sendPromptDefault(classificationQueryContents + question, "gpt-3.5-turbo-1106");
...
async function answerQuestions(value, supplement) {
    for (const questionObj of value) {
        ...
        const improveUserQueryPromptRaw = await fetch(`/api/get-backend-file?file_path=${improveUserQueryPromptFilePath}`, { redirect: 'manual' });
        const improveUserQueryPromptJSON = await improveUserQueryPromptRaw.json();
        ...
        if (questionRevised.startsWith("Answer:")) {
            questionRevised = questionRevised.slice("Answer:".length).trim();
        }
        ...
        if (classification.includes("Analytical Query") || classification.includes("Visual Query")) {
            const response = await sendPromptAgent(supplement, questionRevised, null, null, isTest);
            questionObj.answer = response;
        }
        else if (classification.includes("Contextual Query")) {
            const response = await sendPromptDefault(questionRevised, "gpt-3.5-turbo-1106");
            ...
            const navigationQuestion = supplement + question + activeElementString;
        ...
        const assessmentQuestionFilePath = "gptPrompts/evaluationQuery.txt";
        ...
        const assessment = await sendPromptDefault(assessmentQuestionTemplate, "gpt-3.5-turbo-1106");
