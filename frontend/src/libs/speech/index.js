...
// Scope: support up to two categorical variables and two numerical variables
...
const initialize = (data, options) => {
  ...
  const { title, triggers, responseEl, variables, onLog } = options;
  const hotkeyScope = uniqueId();
  // const speaker = new p5.Speech();// TODO: memory loss? improve this
  ...
  speech.run = () => {
    ...
    hotkeys(
      ...
      (event) => {
        logKeyPresses(listeningKeys, event);
        event.preventDefault();
        const result = processCommand("instructions", data, options);
    ...
    // TODO: remove this?, we may just put the user summary in the alternative text to be consistent with other conditions?
    ...
    hotkeys(
      ...
      (event) => {
        ...
        const result = processCommand("summary", data, options);
  ...
  const instructions = generateInstructions(triggers, title, SETTINGS);
